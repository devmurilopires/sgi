...
class HistoricoView(ctk.CTkFrame):
    ...
    def _criar_date_wrapper(self, parent, width=150):
        container = ctk.CTkFrame(parent, width=width, height=35, fg_color=COLOR_BG, border_width=1, border_color="#AAAAAA", corner_radius=6)
    ...
    def _construir_interface(self):
        ctk.CTkLabel(self, text="Histórico de Exclusões (Auditoria)", font=("Arial Black", 24), text_color=COLOR_PRIMARY).pack(side="top", pady=(10, 5), anchor="w", padx=20)
        ...
        ctk.CTkLabel(f_mod, text="Setor / Módulo", font=("Arial Bold", 11), text_color=COLOR_TEXT).pack(anchor="w")
        # MODIFICAÇÃO: Filtros corretos com a regra de negócio
        self.combo_modulo = CtkParametrosComboBox(f_mod, values=["Todos", "Ponto de Parada", "Itinerário", "Quadro de Horário", "Projetos de Mobilidade"], width=200, height=35, font=("Arial", 12), fg_color=COLOR_BG, border_color="#D1D5DB")
        ...
        ctk.CTkLabel(f_num, text="Nº do Registro", font=("Arial Bold", 11), text_color=COLOR_TEXT).pack(anchor="w")
        self.entry_numero = ctk.CTkEntry(f_num, width=150, height=35, font=("Arial", 12), fg_color=COLOR_BG, border_color="#D1D5DB")
        ...
        ctk.CTkLabel(f_excl, text="Autor da Exclusão", font=("Arial Bold", 11), text_color=COLOR_TEXT).pack(anchor="w")
        self.entry_excl = ctk.CTkEntry(f_excl, width=250, height=35, font=("Arial", 12), fg_color=COLOR_BG, border_color="#D1D5DB")
        ...
        ctk.CTkLabel(f_dt_ini, text="Data Inicial da Exclusão:", font=("Arial Bold", 11), text_color=COLOR_TEXT).pack(anchor="w")
        ...
        ctk.CTkLabel(f_dt_fim, text="Data Final da Exclusão:", font=("Arial Bold", 11), text_color=COLOR_TEXT).pack(anchor="w")
        # CORREÇÃO: O Wrapper agora é empacotado e fica visível!
        ...
        ctk.CTkButton(f_btns, text="🔍 Buscar", fg_color=COLOR_PRIMARY, hover_color=COLOR_HOVER, font=("Arial Bold", 13), width=120, height=35, command=self.acao_buscar).pack(side="left", padx=5)
        ctk.CTkButton(f_btns, text="🧹 Limpar", fg_color="transparent", text_color=COLOR_TEXT, border_width=1, font=("Arial Bold", 13), width=100, height=35, command=self._limpar_filtros).pack(side="left")

        info_frame = ctk.CTkFrame(self, fg_color="transparent")
        info_frame.pack(fill="x", padx=20, pady=(5, 5))
        self.lbl_contador = ctk.CTkLabel(info_frame, text="0 resultados", font=("Arial Bold", 14), text_color=COLOR_TEXT)
        self.lbl_contador.pack(side="left")

        pag_frame = ctk.CTkFrame(info_frame, fg_color="transparent")
        pag_frame.pack(side="right")
        self.btn_ant = ctk.CTkButton(pag_frame, text="<", width=35, height=30, fg_color=COLOR_PRIMARY, hover_color=COLOR_HOVER, font=("Arial Black", 14), command=self._pagina_anterior)
        ...
        self.btn_prox = ctk.CTkButton(pag_frame, text=">", width=35, height=30, fg_color=COLOR_PRIMARY, hover_color=COLOR_HOVER, font=("Arial Black", 14), command=self._proxima_pagina)
        ...
        self.header_frame = ctk.CTkFrame(self.tabela_container, fg_color=COLOR_PRIMARY, corner_radius=6)
    ...
    def _renderizar_pagina(self):
        ...
        for i, linha in enumerate(self.dados_completos[inicio : inicio + self.itens_por_pagina]):
            bg_color = COLOR_BG if i % 2 == 0 else "#FFFFFF"
            ...
            ctk.CTkButton(frame_botoes, text="🔍 Detalhes", fg_color=COLOR_PRIMARY, hover_color=COLOR_HOVER, width=90, height=28, 
    ...
    def _mostrar_detalhes(self, modulo, numero, dados_json):
        ...
        ctk.CTkLabel(popup, text=f"Auditoria Detalhada: {modulo} Nº {numero}", font=("Arial Black", 20), text_color=COLOR_PRIMARY).pack(pady=(20, 5))
        ctk.CTkLabel(popup, text="Abaixo estão os dados originais preservados no momento da exclusão.", font=("Arial", 12), text_color=COLOR_TEXT).pack(pady=(0, 15))
        
        scroll = ctk.CTkScrollableFrame(popup, fg_color=COLOR_BG, border_width=1, border_color=COLOR_SECONDARY, corner_radius=10)
        ...
        if isinstance(dados_json, dict):
            ...
            if dados_autoria:
                frame_autoria = ctk.CTkFrame(scroll, fg_color=COLOR_BG, corner_radius=8, border_width=1, border_color=COLOR_SECONDARY)
                frame_autoria.pack(fill="x", pady=(10, 20), padx=10)
                ctk.CTkLabel(frame_autoria, text="📋 Destaque de Autoria e Responsabilidade Original", font=("Arial Black", 14), text_color=COLOR_PRIMARY).pack(anchor="w", padx=15, pady=(10, 5))
                
                for k, v in dados_autoria.items():
                    lk = ctk.CTkFrame(frame_autoria, fg_color="transparent")
                    lk.pack(fill="x", padx=15, pady=3)
                    
                    ctk.CTkLabel(lk, text=f"{k}:", font=("Arial Bold", 12), width=140, anchor="w", text_color=COLOR_TEXT).pack(side="left")
                    ctk.CTkLabel(lk, text=str(v), font=("Arial", 13), text_color=COLOR_TEXT, anchor="w").pack(side="left")

            # --- CONTEÚDO RESTANTE DO ARQUIVO ---
            ctk.CTkLabel(scroll, text="Todos os Dados do Arquivo", font=("Arial Black", 14), text_color=COLOR_PRIMARY).pack(anchor="w", padx=15, pady=(10, 10))
            ...
            for chave, valor in dados_json.items():
                ...
                ctk.CTkLabel(linha, text=f"{label_chave}:", font=("Arial Bold", 12), width=160, anchor="w", text_color=COLOR_TEXT).pack(side="left", anchor="n")
                
                valor_texto = str(valor) if valor and str(valor) != "None" else "-"
                lbl = ctk.CTkLabel(linha, text=valor_texto, font=("Arial", 13), anchor="w", justify="left", wraplength=400, text_color=COLOR_TEXT)
                lbl.pack(side="left", fill="x", expand=True)
        else:
            ctk.CTkLabel(scroll, text="Dados arquivados em formato bruto incorreto.").pack()

        ctk.CTkButton(popup, text="Fechar Auditoria", fg_color=COLOR_SECONDARY, hover_color=COLOR_HOVER, font=("Arial Bold", 15), height=45, command=popup.destroy).pack(fill="x", padx=40, pady=20)
