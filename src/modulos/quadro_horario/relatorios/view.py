...
class RelatorioQuadroHorarioView(ctk.CTkFrame):
    def __init__(self, master, usuario_logado, tipo_relatorio):
        ...
        # Listas extraídas do contexto específico de Quadro de Horário
        ...
        self.lista_assuntos = ["Todos", "Aumento de frota", "Alteração de itinerário", "Criação de linha", "Desativação de linha", "Extensão de itinerário", "Mudança de ponto final/retorno", "Redução de frota", "Substituição de veículo", "Outros"]
        self.lista_tipos_pesq = ["Todos", "Sobe e Desce", "Frequência Visual", "Sobe e Desce / Frequência Visual", "Volumetria", "Outros"]

        if self.tipo_doc == "PESQUISA":
            self.colunas_config = {
                "id": "ID", "titulo": "Título", "tipo": "Tipo de Pesquisa", 
    ...
    def _construir_interface(self):
        ...
        ctk.CTkLabel(header_filtro, text=f"Filtros de Pesquisa - {self.tipo_doc} (Quadro de Horário)", font=("Arial Black", 16), text_color="#0F8C75").pack(side="left")
        ...
        for key, label in self.colunas_config.items():
            ...
            # --- LÓGICA DOS COMBOS E AUTOCOMPLETES (SPR) ---
            if key == "decisao":
                widget = ctk.CTkComboBox(f, values=["Todos", "DEFERIDO", "INDEFERIDO"], height=35, fg_color="#F9FAFB")
                widget.set("Todos")
            elif key == "assunto":
                widget = ctk.CTkComboBox(f, values=self.lista_assuntos, height=35, fg_color="#F9FAFB")
                widget.set("Todos")
            elif key == "solicitante":
                widget = ctk.CTkComboBox(f, values=self.lista_solicitantes, height=35, fg_color="#F9FAFB")
                widget.set("Todos")
            elif key == "tipo": # Tipo de Pesquisa
                widget = ctk.CTkComboBox(f, values=self.lista_tipos_pesq, height=35, fg_color="#F9FAFB")
                widget.set("Todos")
            elif key == "linhas":
                widget = ctk.CTkComboBox(f, values=self.lista_linhas[:20] if self.lista_linhas else [""], height=35, fg_color="#F9FAFB")
                widget.set("") 
        ...
        ctk.CTkLabel(f_data, text="Período de Criação", font=("Arial Bold", 11), text_color="#666666").pack(anchor="w")
    ...
    def acao_detalhes(self):
        ...
        titulo_num = dado.get('id') if self.tipo_doc == "PESQUISA" else dado.get('numero_parecer_ano')
    ...
    def acao_excluir(self):
        ...
        # Modal específico de exclusão pedindo motivo (Padrão do Quadro de Horário)
        popup = ctk.CTkToplevel(self)
        popup.title("Exclusão de Arquivo")
        ...
        def confirmar():
            ...
            sucesso, msg = self.service.excluir_registro(self.tipo_doc, sel[0], motivo, self.usuario_logado)
            if sucesso:
        ...
        ctk.CTkButton(popup, text="Confirmar Exclusão", width=150, height=40, fg_color="#D32F2F", hover_color="#B71C1C", command=confirmar).pack(pady=20)
