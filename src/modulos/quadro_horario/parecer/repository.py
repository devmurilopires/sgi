...
class ParecerQuadroHorarioRepository:
    ...
    def obter_proximo_numero_parecer(self):
        try:
            with get_db_connection() as conn:
                with conn.cursor() as cur:
                    # REMOVIDO: O filtro de sistema_origem para que a numeração siga a sequência global (ex: 388, 389...)
                    cur.execute("""
    ...
    def salvar_parecer_no_banco(self, dados_db):
        # 1. MODIFICAÇÃO: Blindagem do contexto IN ('PARECER', 'DECISAO_PARECER') e username
        query_base = """
        ...
        try:
            ...
            # MODIFICAÇÃO: Mensagem de erro ajustada para refletir a realidade
            return False, f"Erro relacional: O banco rejeitou a inserção. Verifique se a Origem e a Decisão (Deferido/Indeferido) estão corretamente cadastradas."
        except Exception as e:
